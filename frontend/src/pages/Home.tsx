...
const Home: React.FC = () => {
  ...
  const [singleSelectOption, setSingleSelectOption] = useState<string | null>(null);
  ...
  return (
    <div>
      ...
      <Selector
        ...
        onFilterChange={(selectedOptions) => handleSingleSelectChange(selectedOptions[0])}
        isMultiSelect={false}
      />
      {singleSelectOption && <p>Currently selected (single-select): {singleSelectOption}</p>}
    </div>
  );
};
