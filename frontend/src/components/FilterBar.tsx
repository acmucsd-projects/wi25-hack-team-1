...
const FilterBar: React.FC = () => {
  const [date, setDate] = React.useState<Date | Date[] | (Date | null | undefined)[] | null | undefined>([new Date()]);
  const [departure, setDeparture] = React.useState<{ id: string; label: string }[]>([]);
  const [destination, setDestination] = React.useState<{ id: string; label: string }[]>([]);
  ...
  return (
    ...
    />
      ...
      <Select
        ...
        onChange={(params) => setDeparture(params.value as { id: string; label: string }[])}
      ...
      <Select
        ...
        onChange={(params) => setDestination(params.value as { id: string; label: string }[])}
      />
